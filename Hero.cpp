...
void CHero::Run()
{
	...
	//按攻击
	if(act==0||act==1||act==3||(act==2&&m_CurF>3))
	{
		if(m_KeyState[_K_B]==_KISS_DOWN&&m_KStime[_K_B]==1)
		{//攻击键
			if(preface==2)
				m_CurM=7;
			else if(preface==8)
	...
	//测试游泳
	POINT p={m_x,m_y};
	RECT r={15,803,199,903};
	if(PtInRect(&r,p)&&act!=11&&act!=12)
	{
		if(preface==2)
			m_CurM=37;
		else if(preface==8)
