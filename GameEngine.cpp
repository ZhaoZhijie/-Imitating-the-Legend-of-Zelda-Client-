...
bool CGameEngine::Init(int ClientW, int ClientH)
{
	...
	AdjustWindowRect(&r, /*WS_POPUP | WS_MINIMIZEBOX | WS_SYSMENU, */WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX, FALSE);
	
	//创建窗口
	m_hWnd = CreateWindowA(
		m_WndClass.lpszClassName,
		0,
		/*WS_POPUP | WS_MINIMIZEBOX | WS_SYSMENU,*/ WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX,
		 r.left,
		 r.top,
		 r.right - r.left,
		 r.bottom - r.top,
...
void CGameEngine::Run()
{
	...
	while(msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			m_GO->End(m_CurScene->m_virW,m_CurScene->m_virH);
			//逻辑 和 输入
...
//获取客户端
CTcpClient* CGameEngine::GetClient()
{
	return &m_Client;
}

//获取内存池
CMemoryPool * CGameEngine::GetMemPool()
{
	return &m_MP;
}
