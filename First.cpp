...
CFirst::CFirst(const char * name1, const char * name2)
{
	...
	m_TX.clear();
	m_Monster.clear();
	m_p[0].Init(this,588,250,6);
	m_p[1].Init(this,540,250,6);
	//添加几个怪物做测试
	for(int n=0;n<14;n++)
	{
...
//两个角色y坐标的比较
bool Compare(CRole * r1, CRole * r2)
{
	float x1,y1,x2,y2;
	r1->GetXY(&x1,&y1);
	r2->GetXY(&x2,&y2);
...
bool CFirst::Output(int id, char ** data, int *len,int * myvirX,int * myvirY)
{
	...
	*myvirY=virY;
	...
	for(std::list<extGood>::iterator g=m_extGood.begin();g!=m_extGood.end();g++)
	{
		...
		{
			SinglePic bs={g->id,g->x,g->y+g->z*0.7};
			m_Layer[0].push_back(bs);
		}
	}
	//将血量 金钱等作为特殊特片加入特殊图片链表
	...
	*(short*)&unit[2]=-virX;
	*(short*)&unit[4]=-virY;
	memcpy(*data+10+i*6, unit, 6);
	i++;
	for(int n=0;n<3;n++)
	{
		for(std::list<SinglePic>::iterator k=m_Layer[n].begin();k!=m_Layer[n].end();k++)
		{
			*(unsigned short*)&unit[0]=k->no;
			*(unsigned short*)&unit[2]=k->x-virX;
			*(unsigned short*)&unit[4]=k->y-virY;
			memcpy(*data+10+i*6, unit, 6);
			i++;				
		}
	}
	...
	std::sort<CRole**>(rolelist,rolelist+2,Compare);
	for(int k=0;k<2;k++)
	{
		float x,y,z;
		rolelist[k]->GetXY(&x,&y,&z);
		unsigned short no=rolelist[k]->GetNo();
		unsigned short ix=x;
		unsigned short iy=y+z*0.7;
		//如果有影子先显示影子
		if(m_p[k].GetShade()==0)
		{
			*(unsigned short*)&unit[0]=8;
			*(unsigned short*)&unit[2]=x-virX;
			*(unsigned short*)&unit[4]=y-virY;
			memcpy(*data+10+i*6, unit, 6);
			i++;
		}
		//*(unsigned short*)&unit[0]=no;
		//*(unsigned short*)&unit[2]=ix-virX;
		//*(unsigned short*)&unit[4]=iy-virY;
		//memcpy(*data+10+i*6, unit, 6);
		//i++;

		int site=rolelist[k]->GetSite();
		int act=site%100;
		//如果有举着的东西
		if(act==4||act==5||act==6)
		{
			int curM,curF;
			rolelist[k]->GetAct(&curM,&curF);
			unsigned char lift=((CHero*)rolelist[k])->GetLift();
			int dx=CFirst::GetMoves()->GetBigX(curM,curF);
			int dy=CFirst::GetMoves()->GetBigY(curM,curF);
			*(unsigned short*)&unit[0]=lift;
			*(unsigned short*)&unit[2]=ix+dx-virX;
			*(unsigned short*)&unit[4]=iy+dy-virY;
			memcpy(*data+10+i*6, unit, 6);
			i++;
		}
	}
...
void CFirst::Run()
{
	...
	for(int i=0;i<2;i++)
	{
		...
		for(std::list<extGood>::iterator g=m_extGood.begin();g!=m_extGood.end();)
		{
			...
			fRect rBs={g->x,g->y,g->x+7,g->y+14};
			if(CTools::RectCrash(&r,&rRole,&rBs))
			{
				//获得宝石
				if(g->id!=34)
				{
		...
		for(int k=0;k<4;k++)
		{
			...
			if(gd)
			{
				...
				{
					if(gd->id!=101&&gd->id!=4&&gd->id!=102)
					{
...
void CFirst::InitData()
{
	...
	//初始化物品数据
	m_Goods.EnterGood(_GOOD_GRASS,0,16,16,16);		//草
	m_Goods.EnterGood(_GOOD_KBOX,0,16,16,16);		//箱子开
	m_Goods.EnterGood(_GOOD_GBOX,0,16,16,16);		//箱子关
	m_Goods.EnterGood(_GOOD_GJG,0,16,16,16);			//机关关
	m_Goods.EnterGood(_GOOD_KJG,0,16,16,16);			//机关开
	m_Goods.EnterGood(_GOOD_YST,0,16,16,16);			//圆石头
	m_Goods.EnterGood(_GOOD_GRASSBONE,0,16,16,16);			//圆石头
}
